...
def test_provider_model_switch_persists(client, auth_headers):
    client.put("/agents/providers/anthropic", json={"model": "claude-3.5-haiku"}, headers=auth_headers)
    providers = client.get("/agents/providers", headers=auth_headers).json()
...
def test_test_run_estimates_no_charge(client, auth_headers):
    res = client.post("/agents/test-run", json={"agent_key": "resumeTailoring"}, headers=auth_headers)
    assert res.status_code == 200
    body = res.json()
