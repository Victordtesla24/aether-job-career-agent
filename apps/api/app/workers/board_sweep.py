...
def sweep_user_stretch(
    ...
    *,
    ...
    #
    # Deliberately AFTER the STORM-1 all-paused short-circuit above: when the
    # user has stopped both agents, autopilot is not declining these jobs on
    # fit — it is not running at all — so recording low-fit skip rows there
    # would attribute the silence to the wrong cause AND write rows in the
    # exact pass STORM-1 exists to keep write-free.
