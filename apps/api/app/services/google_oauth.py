...
(:func:`encode_state` / :func:`decode_state`). That token is single-purpose
(``aud="google-oauth-state"``, 10-minute expiry) and deliberately independent
of the app session — it is signed here with PyJWT rather than importing
``app.security`` so this module stays decoupled from the auth layer.

Env (read directly from ``os.environ``, matching the llm_client convention):
``GOOGLE_OAUTH_CLIENT_ID``, ``GOOGLE_OAUTH_CLIENT_SECRET``,
``GOOGLE_OAUTH_REDIRECT_URI``. The state secret reuses ``NEXTAUTH_SECRET``.
"""
...
#: State token audience + lifetime (seconds).
...
def _state_secret() -> str:
    return os.environ.get("NEXTAUTH_SECRET") or "dev-oauth-state-secret"
