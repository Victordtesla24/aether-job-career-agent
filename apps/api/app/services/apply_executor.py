...
def playwright_form_submitter(
    *,
    ...
    ``user_id``/``company`` are needed only by the U5d-4 email-verification
    loop: the code is read from THAT user's own connected Gmail, and the
    employer is named in the manual step raised when it cannot be. Without a
    ``user_id`` (replay, or a direct call) a verification gate is an honest
    manual step rather than anything Aether tries to work around.

    Returns ``{"submitted", "evidencePath", "destination", "filled",
    "unfilled", "mode", "verification"}``. Raises
    ...
    try:
        with sync_playwright() as runner:  # noqa: SIM117 — cleanup handled below
            ...
            try:
                ...
                submitted = bool(signals["submitted"])
        ...
        json.dumps(
            {
                ...
                # Never the code itself — that is the user's secret and it has
                # no business in an evidence file.
...
def _resolve_verification_gate(
    ...
    *,
    ...
    ``verification_code_email``. Never returns a "resolved" gate it did not
    actually resolve.
    """
...
def _submit_and_confirm(
    ...
    *,
    ...
    """Click the form's own submit, clear any code gate, then look for PROOF.

    The order is the whole point: the gate is resolved BEFORE the confirmation
    is looked for, and the confirmation is then looked for exactly as it always
    was — so the only thing that can ever be called a confirmation is still the
    employer's own page saying so.
    """
    ...
    verification: dict[str, Any] | None = None
    if live and submitted and _detect_verification_gate(page):
    ...
    confirmation = _confirmation_signal(page, before_url) if live else None
    return {
        "submitted": submitted,
        "confirmation": confirmation,
        "verification": verification,
    ...
    *,
    ...
    try:
        outcome = submit(
            ...
            evidence_dir=evidence_dir,
            # U5d-4: whose mailbox the employer's verification code may be read
            # from, and whose name the manual step must carry if it cannot be.
            user_id=user_id,
            company=company,
        )
    except ManualStepRequired as exc:
