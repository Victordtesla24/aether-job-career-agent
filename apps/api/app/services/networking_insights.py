...
def upsert_contact(
    ...
    *,
    ...
    ensure_contact_email_unique_index()
...
def ensure_contact_email_unique_index() -> None:
    """Additive unique index so import upsert cannot race-duplicate emails.

    Partial: only rows with a non-null email. Created under the same advisory
    lock family as OutreachTask DDL so concurrent first-hits do not collide.
    """
    with get_connection() as conn:
        with conn.cursor() as cur:
            ...
            cur.execute(
                """
                CREATE UNIQUE INDEX IF NOT EXISTS "Contact_userId_email_lower_uidx"
                ON "Contact" ("userId", (LOWER("email")))
...
def build_crm_summary(user_id: str) -> dict[str, Any]:
    ...
    # Ensure the table exists up front. A missing table must not silently look
    # like "you have no outreach" — we create it, then query for real.
...
def network_snapshot_for_prompt(user_id: str | None) -> str:
    ...
    if not user_id:
        return (
            "NETWORK SNAPSHOT: contacts: not measured; last contact update: "
            "not measured. No contact emails or names are included."
        )
    try:
        snap = build_analytics_snapshot(user_id)
    except Exception:
        # Honesty law: a failed read is "not measured", never a fabricated 0.
        return (
            "NETWORK SNAPSHOT: contacts: not measured; last contact update: "
            "not measured. No contact emails or names are included."
        )
