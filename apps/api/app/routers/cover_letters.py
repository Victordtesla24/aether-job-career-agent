...
def export_cover_letter_pdf(letter_id: str, current_user: CurrentUser) -> Response:
    """Render the letter as a real single-page PDF (Export PDF, cl12)."""
    ...
    width, height = A4
    margin, line_height = 25 * mm, 5.4 * mm
    max_chars = 92
    y = height - margin

    def _write(line: str, font: str = "Helvetica", size: int = 10) -> None:
        nonlocal y
        if y < margin:
            page.showPage()
            y = height - margin
    ...
    title = f"Cover Letter — {(job or {}).get('title', 'Application')}"
    if job:
        title += f" · {job['company']}"
    _write(title, "Helvetica-Bold", 13)
    y -= line_height / 2
    for paragraph in (letter["coverLetter"] or "").split("\n"):
