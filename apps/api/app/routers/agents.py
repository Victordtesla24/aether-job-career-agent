...
def _begin_anthropic_oauth(user_id: str) -> str:
    ...
    below and the per-user mint at ``/user/providers/anthropic/oauth/mint/start``).
...
def anthropic_oauth_start(current_user: AdminUser) -> dict[str, Any]:
    ...
    per-user mint at ``POST /agents/user/providers/anthropic/oauth/mint/start``.
    """
...
#
# RUN-20260818T0223Z merge note (FEAT-PROVIDER deploy-merge): mounted at
# ``/user/providers/anthropic/oauth/mint/{start,exchange}`` — moved one path
# segment from the pre-merge ``/user/providers/anthropic/oauth/{start,
# exchange}`` — because GAP-PROVIDER-OAUTH-1 (below) independently claimed
# the literal ``/user/providers/{provider}/oauth/{start,exchange}`` path for
# a DIFFERENT contract (auto-persist, masked response, no raw token in the
# body) for the SAME provider id. FastAPI/Starlette match routes in
# registration order — first match wins — so two handlers cannot both own
# one literal path; keeping the mint contract exactly as security-reviewed
# and shipping the newer generic connect at its own authored path (unchanged
# from GAP-PROVIDER-OAUTH-1's own tests) was the only way to ship both
# without silently dropping either. ``test_user_anthropic_oauth_mint.py``'s
# two path constants were updated to match (its assertions/behaviour are
# byte-for-byte unchanged). See docs/delivery/evidence/RUN-20260818T0223Z/
# FEAT-PROVIDER/09-deploy-merge-resolution.md.
# ---------------------------------------------------------------------------


@router.post("/user/providers/anthropic/oauth/mint/start")
def user_anthropic_oauth_start(current_user: CurrentUser) -> dict[str, Any]:
    """Begin the customer's own Connect-with-Anthropic MINT flow.
...
@router.post("/user/providers/anthropic/oauth/mint/exchange")
...
def user_provider_oauth_callback(
    ...
    # RUN-20260818T0223Z merge note: do NOT sync CLAUDE_CODE_OAUTH_TOKEN here,
    # even for an oauth_token credential. That env var is the operator's
    # restart-survival copy of the DEPLOYMENT-WIDE credential (GAP-P7-DEF-A
    # §3.3), written only by the admin-only ``PUT /agents/providers/{id}/
    # credential`` route and ``anthropic_oauth.persist_tokens``. Syncing a
    # customer's own token into it would re-bill every bare ``claude-*`` run
    # — including cron — to this customer once the process restarts, which is
    # the exact F-01 failure mode ``put_user_credential`` was hardened against
    # (see its own "Do NOT sync" comment above). This per-user route must not
    # regress that fix through a different door.
    try:
...
def user_provider_oauth_exchange(
    ...
    try:
        ...
        raise HTTPException(status.HTTP_503_SERVICE_UNAVAILABLE, str(exc)) from exc
    # RUN-20260818T0223Z merge note: do NOT sync CLAUDE_CODE_OAUTH_TOKEN here —
    # see the identical note in ``user_provider_oauth_callback`` above.
    try:
