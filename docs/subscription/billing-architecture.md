...
- **No privilege/role column on `User` (at design time)** (`probe-17`: `isAdmin`/`role` absent). **[AS-BUILT: Cluster F shipped** — additive `isAdmin` (default false), `suspended`, `lastLoginAt` columns on `User` + the `AdminSetting` table + credential rotation now exist in production (the seeded demo account is demoted to `isAdmin=false`); admin panel Tier 1 is live, formal closure pending the operator's own admin credential. **See the BLOCKER-001 security notice in `docs/subscription/admin-guide.md`: the demotion did not stop the demo *identifier* from resolving to the owner account — fixed in code, secret rotation still operator-gated.**]**
